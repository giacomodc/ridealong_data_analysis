...
transf_stops_data <- function(source_folder_name, data_name){
  ...
  stops$parking_type[stops$parking_type == "L"] <- "commercial loading zone"
...
visualize_routes <- function(route_data, stops_data){
...
visualize_segments <- function(seg_data, stops_data){
...
map_route_groc <- visualize_routes(route_groc, stops_groc)
map_route_med <- visualize_routes(route_med, stops_med)

# Execute visualizations for segments on route
map_segm_groc <- visualize_segments(segm_groc, stops_groc)
map_segm_med <- visualize_segments(segm_med, stops_med)
...
# plot comparisons
plot_comp_time <- ggplot(aes(x= mode, y = time_span, fill = mode), data = summary.data) + geom_boxplot() + theme_light() + ylab("time in minutes") + xlab("process step") + ggtitle("Time span distributions in delivery modes")
plot_comp_distance <- ggplot(aes(x= mode, y = distance_covered, fill = mode), data = summary.data) + geom_boxplot() + theme_light() + ylab("distance in km") + xlab("process step") + ggtitle("Distance distributions in delivery modes")
# plot deliveries
plot_deliver_time <- (ggplot(aes(x = index, y = time_span, fill = distance_covered), data = summary.deliveries)
                    + geom_bar(stat= "identity") + ylab("delivery time / parking time in mins") 
                    + theme_light() + ggtitle("Delivery time spans"))
plot_deliver_dist <- (ggplot(aes(x = index, y = distance_covered, fill = time_span), data = summary.deliveries)
                    ...
                    + theme_light() + ggtitle("Walking distances"))
...
plot_meds <- grid.arrange(plot_comp_time, plot_comp_distance, plot_deliver_time, 
                          plot_deliver_dist, plot_drive_time, plot_drive_dist,
                          nrow=2, ncol = 3)
plot_meds
